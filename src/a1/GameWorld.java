...
public class GameWorld {
	...
	public void scoop(){ 
		...
		for(int i = 0; i < gObjects.size(); i++){
			if(gObjects.get(i)instanceof Cat){
				...
				if((((Cat) gObjects.get(i)).getXLoc() > netLeftB) && (((Cat) gObjects.get(i)).getXLoc() < netRightB) && 
					(((Cat) gObjects.get(i)).getYLoc() > netBotB) && (((Cat) gObjects.get(i)).getYLoc() < netTopB)){
				...
				if((((Dog) gObjects.get(i)).getXLoc() > netLeftB) && (((Dog) gObjects.get(i)).getXLoc() < netRightB) && 
				  (((Dog) gObjects.get(i)).getYLoc() > netBotB) && (((Dog) gObjects.get(i)).getYLoc() < netTopB)){
	...
	public void fightCollision(){
		...
		//RANDOM INTEGER BASED ON SIZE OF DOG LIST ---  NEED THIS TO GET VALUE FROM DOGLIST
		...
		for(int i = 0; i < gObjects.size(); i++){
			if(gObjects.get(i)instanceof Dog){
				if(((Dog) gObjects.get(i)).getScratches() > 0){
					...
					totalPoints += -1*(((Dog) gObjects.get(i)).getScratches());
				}
			}
		}
